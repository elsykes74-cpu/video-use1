#!/usr/bin/env python3
"""Narration only — script.txt -> voice.mp3 via free Edge TTS.

Deliberately does one job with no ffmpeg dependency, so it fails fast and
loudly if the TTS endpoint is unreachable rather than dying 20 minutes into
a render. Applies the manifest's pronunciation_fixes first and writes
script_spoken.txt so you can see exactly what the voice was given.
