...
- Archival MJ photos by decade, stored in Drive under MJ Videos / Reference Images by Decade
- - Concert or performance clips where rights allow
  - - Gemini-generated mood and B-roll only (stages, lighting, fashion, crowd) -- no AI-generated MJ face, matching the design already in mj-niche.yaml
    ...
    - Music: duck_factor 0.10 from mj-niche.yaml (music at 10 percent under narration). OPEN ITEM -- confirm this matches "1.5 volume" from the brief; need the actual render or EDL param name to lock it in exactly.
    ...
    - - Fast-paced, punchy cuts, matching mj-niche.yaml pacing -- quicker cadence than the Elvis vertical.
      - - Caption reveal should hit on the beat if music is present.
        - - Every photo needs a confirmed-rights source -- pull only from the Drive reference folders you populate yourself.
          - - No synthetic MJ face or voice, ever. B-roll and atmosphere only from Gemini; real archival photos for anything showing him.
           
            - ## Open items before this is fully wired
           
            - 1. QuickKick duration and volume params -- need the QuickKick service source (not in this repo) to confirm how to lock the 70s runtime and exact music level.
              2. 2. TikTok upload leg -- this pipeline auto-uploads to YouTube via OAuth; TikTok posting is still a manual step or a separate integration.
